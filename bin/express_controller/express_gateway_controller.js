...
class GatewayExpressController extends ExpressController {
    ...
    _initialiseProxy() {
        ...
        this._oProxy.on('proxyRes', function (proxyRes, req, res) {
            console.log(`Request: ${req.url} Code: ${res.statusCode}-${res.statusMessage}`);
        });
    }
    ...
    /**
     * Lifehook to register additional server events
     * @param {*} poServer 
     */
     onServerListen (poServer) {
        poServer.on('upgrade', (req,res) => {
            if(req.url.startsWith("/sap/bc/apc/")) {
